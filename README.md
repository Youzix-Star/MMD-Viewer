...
| **アイドル**（舞蹈，含 BGM 联动） | 卜逻desu | **动作：卜逻desu · 编舞：宇宙兔兔海**（硬性要求） | 基于 Sour式初音 Ver.1.00 制作——**White 完美适配**（⚠ 禁止二次配布，不入库本地自备） |
| **アイドル镜头**（相机动作） | 卜逻desu | 同上 | 配合アイドル 舞蹈使用 |
...
- アイドル 动作（卜逻desu）：**未经允许禁止二次配布**、商用需联系、禁 18+——**不入库**（本地自备，见 CHANGE.md）
- 曾接入的美丽Liya 动作包（眨眼/瞳孔/表情/镜头）因条款含"禁手机mmd/禁二传"**已全删**（2026-08-15，见 CHANGE.md）

### 自行添加动作（2026-08-16 起支持）

动作清单在 `manifest.json` 的 `actions` 数组（与模型机制对称），**使用者可自行添加**：

1. 把动作 VMD 放入任意目录（如 `models/` 下新建文件夹）
2. 在 `manifest.json` 的 `actions` 加一条：
   ```json
   { "id": "my-motion", "name": "显示名", "file": "models/我的动作/动作.vmd", "bgm": "models/我的动作/BGM.wav", "camera": false }
   ```
   - `bgm` 可选（有则动作播放时联动，无则静音）；`camera: true` 表示相机动作（运镜）
3. 刷新页面，MODELS 面板「动作」区即出现

> ⚠️ **条款自负**：自行添加的动作，配布条款与借物表由使用者自行确认（本项目入库动作的条款见上表）。
> 动作需要模型带相应辅助骨骼（如肩P/腕捩/足D）才能完整发挥；文件名中文/空格均可，HTTP 自动编码。
