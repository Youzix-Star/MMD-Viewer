...
## 🔄 2026-08-20 · 回滚：Miuix 代码改动全部撤销

### 回滚内容

用户指令：将项目整体回档到没有 Miuix 代码改动的状态（保留文档记录）。

**已撤销的提交**（按时间倒序）：
- `dd18f8d` feat: Compose UI Phase 3——风向罗盘 + 动作选择
- `bbd9dda` feat: Compose UI Phase 2——3D 手势桥接 + HUD + 模型面板
- `7000756` chore: gitignore
- `1741ea9` feat: Compose Web UI 替换 Phase 1——引擎抽取 + MmdEngine API
- `de07b1d` feat: Miuix 风格迁移——CSS Token + TopAppBar/Switch

**保留的内容**：
- `f8ea4c0` docs: Miuix 探索记录 + 璇玑交接（文档完整保留）
- 所有 Miuix 之前的代码（模型导入/亮度调节/日志时间戳等）

### 回滚原因

- Miuix 迁移需要 PC 构建环境（Termux 不兼容 Gradle）
- 工作量评估为几天到几周，超出当前范围
- 设计规范已提取在文档中，供后续开发者参考

### 当前项目状态

- **代码**：模型导入 + 亮度调节 + 日志时间戳 + 动态文件夹
- **文档**：Miuix 设计规范 + 璇玑交接 + 完整历史
- **最新提交**：`f8ea4c0`（文档交接）
...
**代码**：MD2 原始版 + 引擎拆分（index.html 825 行 + mmd-engine.js 1914 行）
**功能**：物理模拟 / 动作播放 / 模型导入 / 12个实验性功能（色调映射/辉光/雾效/线框/轮廓/阴影/SMAA/背景色/旋转速度/惯性/FOV/平移）
**已知待修**：模型加载"先身体后衣服"（B1 回滚，需研究 MMDLoader 纹理加载机制）
**文档**：CHANGE.md（1500+行）+ DEVLOG.md（31章）+ MIUIX-SPEC.md

### 给瑶光的三句话

1. 代码结构干净：引擎（mmd-engine.js）和 UI（index.html）已分离，每个功能都有 commit 记录可追溯。
2. "先身体后衣服"是最大的未解技术问题——需要研究 MMDLoader 内部纹理加载机制，可能需要 hook texManager 的加载时机。
3. 实验性功能是亮点——Three.js post-processing 通过 importmap 直接可用，实现成本极低，可以继续扩展。

### 交接清单

① `git log --oneline -20` 确认代码 ② 读 `CHANGE.md` 最新章 ③ 读 `DEVLOG.md` 三十一章
④ 本地 `python3 -m http.server 8080` 实测 ⑤ 有事查文档、记文档，别靠猜。
